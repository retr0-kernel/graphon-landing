...
export const COMPONENTS = [
  {
    ...
    type: 'DaemonSet',
    port: '—',
    description: 'Runs on every node. Attaches eBPF probes to tcp_connect and tcp_accept syscalls. Emits JSON events to the backend over HTTP.',
  },
  {
    name: 'graphon-backend',
    type: 'Deployment',
    port: '8080',
    description: 'Go + Fiber REST API. Houses the License Engine, RBAC middleware, OIDC handler, Webhook Dispatcher, Snapshot Engine, and background Scheduler.',
  },
  {
    name: 'graphon-ui',
    type: 'Deployment',
    port: '80',
    description: 'React SPA served as static files. Uses @xyflow/react for the live graph canvas. Talks only to graphon-backend via REST.',
  },
  {
    name: 'PostgreSQL',
    type: 'StatefulSet',
    port: '5432',
    description: 'Stores tenants, clusters, API keys, sessions, snapshots, review items, ownership records, drift baselines, and Slack configs.',
  },
  {
    name: 'Neo4j',
    type: 'StatefulSet',
    port: '7687',
    description: 'Stores the live service graph (SERVICE nodes + CALLS edges). All graph queries (path-finding, blast radius, stale detection) run here.',
  },
];

export const DATA_FLOW = [
  { step: '1', title: 'eBPF capture', desc: 'graphon-bpf attaches to tcp_connect/accept on every node. Each observed connection becomes a JSON event: {source, target, namespace, protocol, port}.' },
  { step: '2', title: 'Ingest', desc: 'Events are POSTed to POST /api/v1/ingest/events. The backend writes them to Neo4j as (source)-[:CALLS]->(target) edges and to PostgreSQL as ingestion_events rows.' },
  { step: '3', title: 'Graph queries', desc: 'The UI calls GET /api/v1/graph to fetch the live service topology. Backend queries Neo4j with Cypher and returns a {nodes, edges} JSON response.' },
  { step: '4', title: 'Background jobs', desc: 'The Scheduler runs five goroutines: orphan scan (15 min), drift detection (10 min), baseline promotion (1 h), cleanup scan (1 h), and scheduled snapshots (6 h, Enterprise only).' },
  { step: '5', title: 'Alerts', desc: 'When orphans or drift are detected, the notify.Sender fires a Slack incoming webhook message, and a ReviewItem is created in PostgreSQL for the UI to surface.' },
];
...
export const ENV_VARS: readonly EnvVar[] = [
  ...
  { name: 'GRAPHON_LICENSE_KEY',       default: '—',          required: false, description: 'RS256-signed JWT for Enterprise features. Leave empty for Free.'   },
  ...
  { name: 'LICENSE_GRACE_PERIOD_DAYS', default: '14',         required: false, description: 'Days to keep Enterprise features after license expiry.'            },
] as const;
...
export const DEPLOYMENT_MODES = [
  {
    ...
    desc: 'Default. No license key required. Core graph, ownership, drift, manual snapshots, and basic export (Mermaid/DOT/SVG). Up to 3 users.',
  ...
  {
    mode: 'Enterprise Self-Hosted',
    env: 'GRAPHON_LICENSE_KEY=<jwt>',
    desc: 'RS256-signed license JWT unlocks team-scale features. Self-hosted in your own cluster. Annual license.',
    features: ['Everything in Free', 'OIDC/SSO', 'RBAC (6 roles)', 'Scheduled snapshots (6h)', 'Snapshot retention >30 days', 'GitHub/GitLab PR impact comments', 'Draw.io export', 'Multi-cluster (up to license limit)', 'Priority support'],
  },
  {
    ...
    features: ['Everything in Enterprise', 'Managed Neo4j + PostgreSQL', '5-minute onboarding', 'Dedicated SLA', 'SOC 2 roadmap'],
  },
];
...
export const EXPORT_ENDPOINTS: readonly ApiEndpoint[] = [
  { method: 'POST', path: '/api/v1/export', auth: 'tenant+cluster', description: 'Export the graph. Body: {format}. Free: mermaid|dot|svg. Enterprise adds: drawio.' },
];
...
export const RBAC_ROLES: readonly RbacRow[] = [
  { role: 'viewer',         permissions: 'graph:read, ownership:read, snapshot:read, export:read, search:read',                                           typical: 'On-call engineers, read-only stakeholders' },
  ...
  { role: 'manager',        permissions: '+ graph:write, snapshot:write, drift:write',                                                                    typical: 'Team leads setting baselines and capturing snapshots' },
  { role: 'platform-admin', permissions: '+ cluster:read/register/revoke, audit:read',                                                                    typical: 'Platform engineers managing cluster fleet'  },
...
export const APPLY_KEY_LINES: readonly TerminalLine[] = [
  { type: 'comment', text: '# Apply your Enterprise license key' },
  ...
  { type: 'success', text: '{"plan":"enterprise","features":[...],"limits":{"clusters":10,...}}' },
  { type: 'comment', text: '# Or via Helm values at deploy time:' },
...
export const FREE_FEATURES_TABLE = [
  { feature: 'Live dependency graph',     free: true,    enterprise: true  },
  { feature: 'Service search',            free: true,    enterprise: true  },
  { feature: 'Ownership CRUD',            free: true,    enterprise: true  },
  { feature: 'Orphan & drift detection',  free: true,    enterprise: true  },
  { feature: 'Safe-delete analysis',      free: true,    enterprise: true  },
  { feature: 'Manual snapshots + diff',   free: true,    enterprise: true  },
  { feature: 'Export (Mermaid/DOT/SVG)',  free: true,    enterprise: true  },
  { feature: 'Slack governance alerts',   free: true,    enterprise: true  },
  { feature: 'REST API + API keys',       free: true,    enterprise: true  },
  { feature: 'Users',                     free: 'Up to 3', enterprise: 'Unlimited' },
  { feature: 'Snapshot retention',        free: '30 days', enterprise: 'Custom'    },
  { feature: 'OIDC/SSO',                  free: false,   enterprise: true  },
  { feature: 'RBAC (6 roles)',            free: false,   enterprise: true  },
  { feature: 'Scheduled snapshots (6h)',  free: false,   enterprise: true  },
  { feature: 'GitHub/GitLab PR comments', free: false,   enterprise: true  },
  { feature: 'Draw.io export',            free: false,   enterprise: true  },
  { feature: 'Multi-cluster',             free: false,   enterprise: true  },
  { feature: 'Priority support',          free: false,   enterprise: true  },
];
...
export const HELM_INSTALL_LINES: readonly TerminalLine[] = [
  ...
  { type: 'comment', text: '# Enterprise install with OIDC + license key' },
  { type: 'command', text: 'helm upgrade --install graphon graphon/graphon \\' },
  { type: 'output',  text: '  --namespace graphon --create-namespace \\' },
...
export const HELM_VALUES: readonly HelmValue[] = [
  ...
  { key: 'backend.licenseKey',         default: '""',            description: 'Enterprise license JWT. Leave empty for Free tier.'                 },
  ...
  { key: 'backend.github.webhookSecret', default: '""',          description: 'HMAC secret for GitHub webhook verification.'                       },
  { key: 'backend.gitlab.token',       default: '""',            description: 'GitLab PAT for posting MR notes.'                                   },
  { key: 'backend.gitlab.webhookSecret', default: '""',          description: 'Token for GitLab webhook header verification.'                      },
  { key: 'backend.slack.webhookURL',   default: '""',            description: 'Slack incoming webhook URL for governance alerts.'                  },
  { key: 'ui.image.tag',               default: 'latest',        description: 'graphon-ui image tag.'                                              },
  { key: 'bpf.image.tag',              default: 'latest',        description: 'graphon-bpf agent image tag.'                                       },
