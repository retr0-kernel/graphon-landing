...
| `tcp_l7.bpf.c` | `sock_ops` | First-bytes L7 sampling — ClientHello for TLS fingerprint (JA3), HTTP/2 frame parsing (Pro). |
...
pkg/otlp       OTLP/HTTP listener on :8081 (so workloads can push their own telemetry)
```
...
A note on **tier-aware sanitisation**. The Free tier ships edges + protocol + lifecycle metrics. The Pro Self-Hosted tier wires through TLS handshake metadata, JA3 server name, HTTP/2 stream counts, retransmit classification (fast / tail / timeout / SACK-DUP), and encrypted-bytes attribution. The cleanest place to strip those fields is the very last call site, in `pkg/agent`, where `sanitizeForTier()` rewrites the JSON bytes before the HTTP POST. No other code in the stack has to know about the tier.
...
If you want to read the source, it's all [open](https://github.com/retr0-kernel) under Apache-2.0 — `graphon-bpf` for the agent, `graphon-backend` for the API + storage adapters, `graphon-helm` for the chart, and `graphon-landing` for this site.
